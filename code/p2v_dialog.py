...
class P2VConverterGUI:
    ...
    def create_main_frame(self):
        ...
        # Secondary tools row (QCOW2 Resize, Format Converter, LUKS Cipher, Delete Files)
        ...
        # Tools in two rows for better responsiveness
        ...
        convert_btn.grid(row=0, column=1, sticky="ew")
        
        tools_row2_frame = ttk.Frame(vm_config_frame)
        tools_row2_frame.grid(row=4, column=0, columnspan=2, sticky="ew", padx=(10, 0))
        tools_row2_frame.grid_columnconfigure(0, weight=1)
        tools_row2_frame.grid_columnconfigure(1, weight=1)
        
        cipher_btn = ttk.Button(tools_row2_frame, text="LUKS Encryption", 
                            command=self.open_luks_ciphering)
        cipher_btn.grid(row=0, column=0, padx=(0, 5), sticky="ew")
        
        virt_btn = ttk.Button(tools_row2_frame, text="Virt-Manager", 
                            command=self.open_virt_manager)
        virt_btn.grid(row=0, column=1, sticky="ew")
        
        # Tools description
        tools_info_label = ttk.Label(vm_config_frame, 
                                    text="Resize QCOW2 images • Convert disk formats • Encrypt/decrypt with LUKS • Cleanup files",
                                    font=("Arial", 8), foreground="gray")
        tools_info_label.grid(row=5, column=0, columnspan=2, sticky="w", pady=(5, 0))
    ...
    def open_delete_files_manager(self):
            ...
            try:
                ...
                if stats['removed'] > 0 or stats['failed'] > 0:
                    summary_msg = f"File Deletion Summary: {stats['removed']} deleted, {stats['failed']} failed"
